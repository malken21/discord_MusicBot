...
const searchVideo = (searchText) => {
  const youtube = YouTube.Search.list("snippet", { q: searchText, maxResults: 1, safeSearch: "strict", regionCode: "JP", type: "video" });
...
const searchList = (searchText) => {
  const youtube = YouTube.Search.list("snippet", { q: searchText, maxResults: 1, safeSearch: "strict", regionCode: "JP", type: "playlist" });
  const item = youtube.items[0];
...
const getList = (listId) => {
  const youtube = YouTube.PlaylistItems.list("snippet,contentDetails,status", { playlistId: listId, pageToken: "" });
  let result = [];
  ...
  for (let loop = 0; page.nextPageToken != undefined; loop++) {
    page = YouTube.PlaylistItems.list("snippet,contentDetails,status", { playlistId: listId, pageToken: page.nextPageToken });
